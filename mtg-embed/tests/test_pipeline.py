...
def test_unchanged_chunk_is_never_sent_to_embedder():
    ...
    # Second run: nothing changed, so encode() must not be called again.
    embed_and_store(chunks, store, embedder)
    assert model.encode_calls == 1
...
def test_only_changed_chunk_text_is_sent_to_embedder():
    ...
    embed_and_store([_chunk(unchanged_id, "h1"), _chunk(changed_id, "h2")], store, embedder)
    seen_texts.clear()

    summary = embed_and_store(
        [_chunk(unchanged_id, "h1"), _chunk(changed_id, "h2-changed")], store, embedder
    )
...
def test_retrieve_batch_size_controls_batching():
    ...
    summary = embed_and_store(chunks, store, embedder, retrieve_batch_size=1)
